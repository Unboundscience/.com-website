...
export function useScrollVideoGrowth(sectionRef) {
  ...
  const drawFrame = useCallback((index) => {
    ...
    const frames = images.growth

    if (!canvas || !ctx || !frames || !frames[index] || !frames[index].complete || !frames[index].naturalWidth) return
  ...
  useEffect(() => {
    if (!sectionRef.current || !images.growth) return
  ...
  const initCanvas = useCallback((canvas) => {
    ...
    if (images.growth && images.growth[0] && images.growth[0].complete) {
      drawFrame(0)
